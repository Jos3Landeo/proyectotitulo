...
public class Login extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (us.getEmail() != null) {
            ...
            carga.cargaInicial(request, response); //Carga inicial por usuario
            carga.menuTablero(request, response); //Carga del menu tablero principal
            
            //Para las tareas
            sesion.setAttribute("TareaAgregada", "hide");

            response.sendRedirect(request.getContextPath() + "/autorizado/tablero");
        } else {
            request.setAttribute("noEncontrado", "Usuario y contraseña no coinciden");
            String jsplogin = "/login.jsp";
            request.getRequestDispatcher(jsplogin).forward(request, response);
